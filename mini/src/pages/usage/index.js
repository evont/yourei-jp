...
class Usage extends Component {
  ...
  render() {
    ...
    const { relative, numExp = 0, sentences } = data;
    let list;
    if (sentences.length) {
      list = sentences.map((ele, ind) => {
        return <View key={ind} className='item'>
          <Text className='item-sencence'>{ele.sentence}</Text>
        </View>
      })
    } else {
      list = <View className='holder'>
        <Text className='holder-text'>没有相关用例</Text>
      </View>
    }
    return (
        <View className='usage'>
          ...
          <View className='usage-sentence'>
              <View className='head'>
                <Text className='head-text'>用例【共{numExp}条】</Text>
              </View>
              <View>
                <View>
                  { list }
                </View>
                <View><Text>点击加载更多</Text></View>
              </View>
          </View>
      </View>
    )
